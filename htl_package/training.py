...
def _make_finetune_split(
    ...
    """
    Hold out a validation split for fine-tuning.

    Group-aware when ``doi`` is available (rows sharing a doi never straddle
    train/val), otherwise a plain random row split.
    """
    rng = np.random.RandomState(seed)
    ...
    if "doi" in df.columns:
        ...
        perm = rng.permutation(n)
        n_va = max(1, int(round(n * val_ratio)))
        n_va = min(n_va, n - 1)  # always keep at least one training group

        va_groups = set(unique_dois[perm[:n_va]])
        dois = df["doi"].values
        va_idx = np.array([i for i, d in enumerate(dois) if d in va_groups])
        tr_idx = np.array([i for i, d in enumerate(dois) if d not in va_groups])
    else:
        idx = np.arange(len(df))
        tr_idx, va_idx = train_test_split(idx,
                                          test_size=val_ratio,
                                          random_state=seed)

    logger.info(
        f"Fine-tune hold-out: {len(tr_idx)} train / {len(va_idx)} val pairs"
    )
...
def finetune(config: FinetuneConfig) -> Dict[str, Any]:
    """
    Fine-tuning mode: Load best model weights, continue training with
    early stopping on a held-out validation split, save as final model.
    """
    ...
    use_val = 0.0 < config.val_ratio < 1.0 and len(df) > 1
    if use_val:
        tr_idx, va_idx = _make_finetune_split(df, config.val_ratio, config.seed)
        tr_df, va_df = df.iloc[tr_idx], df.iloc[va_idx]
    else:
        logger.warning(
            "val_ratio is 0 (or dataset too small) — fine-tuning without "
            "validation monitoring; early stopping is disabled."
        )
        tr_df, va_df = df, None

    tr_ds = PairDataset(tr_df, scaler=saved_scaler, fit_scaler=False)
    ...
    for epoch in range(1, config.finetune_epochs + 1):
        tr_loss, s1, s2, y1, y2 = _run_epoch(model, tr_loader, criterion,
                                             optimizer)
        tr_met = _metrics(s1, s2, y1, y2)

        if va_loader is not None:
            va_loss, sv1, sv2, yv1, yv2 = _run_epoch(model, va_loader, criterion)
            va_met = _metrics(sv1, sv2, yv1, yv2)
        else:
            va_loss, va_met = tr_loss, tr_met
        ...
        logger.info(
            f"Ep {epoch:4d} | tr={tr_loss:.4f} "
            f"(acc={tr_met['PCE_pair_acc']:.3f} ρ={tr_met['PCE_spearman']:.3f}) | "
            f"va={va_loss:.4f} "
            f"(acc={va_met['PCE_pair_acc']:.3f} ρ={va_met['PCE_spearman']:.3f})"
        )

        if va_loader is not None and stopper.step(va_loss, model):
    ...
    if va_loader is not None:
        ...
        logger.info(
            f"Restored best weights from epoch {best_ep} "
