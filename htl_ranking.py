...
if __name__ == "__main__":
    ...
    p.add_argument(
        ...
        type=float,
        default=None,
        help="Fraction held out as validation during fine-tuning so the run is "
             "monitored instead of blind. Set 0 to train on all data. Default: 0.1.",
    )
    p.add_argument(
        "--finetune_patience",
        type=int,
        default=None,
        help="Fine-tuning early stopping patience (epochs without val improvement). "
             "Default: 10.",
    )
...
# Fine-tuning
python htl_ranking.py --mode finetune --csv htl-data-combinations.csv --checkpoint_dir checkpoints --checkpoint_name best_model --finetune_epochs 20 --finetune_lr 1e-5
