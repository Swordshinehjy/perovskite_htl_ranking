...
Before using this model, it is necessary to clarify the methodological positioning of this task:

- **The candidate space is derived from the known chemical space, not entirely new chemistry.** The new structures to be evaluated are basically variants of alkyl chains, substituents, and functional groups with known skeletons such as PACz/carbazole, sharing the same chemical family as the training materials. If it is a completely new molecule different from the known system, the model performance will inevitably drop.
...
python htl_ranking.py --mode finetune --csv htl-data-combinations.csv \
    ...
    --finetune_epochs 20 --finetune_lr 1e-5

# Hold out 10% as validation so fine-tuning is monitored (early stopping, patience=10)
# Optional overrides: --finetune_val_ratio 0.15 --finetune_patience 8
```
